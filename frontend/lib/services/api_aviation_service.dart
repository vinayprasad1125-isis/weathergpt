...
class ApiAviationService {
  static Future<List<dynamic>> getMetar(String station) async {
    final response = await http.get(Uri.parse('${Config.apiBaseUrl}/api/v1/aviation/metar?station=$station'));
  ...
  static Future<List<dynamic>> getTaf(String station) async {
    final response = await http.get(Uri.parse('${Config.apiBaseUrl}/api/v1/aviation/taf?station=$station'));
    if (response.statusCode == 200) {
      return json.decode(response.body);
    } else {
